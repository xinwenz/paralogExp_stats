...
ggplot(data=xx) + 
    ...
    theme(
        ...
        plot.title = element_text(size = 11, face = "bold", hjust=0.05),
...
p2 <- plotonechrome(twoR,"2R",tg="none")
p3 <- plotonechrome(threeL,"3L",tg="none")
p4 <- plotonechrome(threeR,"3R",tg="none")
...
g_legend<-function(a.gplot){
    ...
    return(legend)}
com_leg <- g_legend(plotonechrome(X,"X",tg="bottom"))
...
tx1=textGrob("Positions of the genes", gp=gpar(fontface="bold", fontsize=15))

pa <- grid.arrange(p1,p2,p3,p4,p5,com_leg,nrow=6,heights=c(2,2,2,2,2.2,0.8), top=tx1)

ggsave(file="~/cloud/para/gene_pos/fig1_gene_positions.pdf",pa, device = "pdf", width = 30, height = 36, units = "cm",dpi=200)

#pdf(pa, file="test.pdf")

#ggarrange(p1,p2,p3,p4,p5,ncol=1,bottom=tx3, common.legend = TRUE, legend ="bottom")
