...
pexp16 <- read.table('16_gene_paralog.txt', header=T)
#exp35$copy <- as.character(exp35$copy)

plotpara <- function(x) {
    ggplot(data=pexp16) + 
        geom_bar(aes(x=paralog, y = ncount, fill=strain, alpha=condition), stat='identity', color="black",position = "dodge",data=pexp16[pexp16$Gene == x,]) +
        scale_fill_manual(values = c('A3'='pink','A4'='purple'))+
