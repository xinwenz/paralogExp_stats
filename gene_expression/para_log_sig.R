...
sig_df <- read.table('para_log_sig.txt', header=T)

for (i in 14:14) {
 ans <- binom.test(x=round(sig_df[i,"count_n1"]), n = round(sig_df[i,"count_n1"] + sig_df[i,"count_n2"]), p=0.5)
 #print(sig_df[i,"count_n1"]) 
 print(c(i,round(ans$p.value,digits = 5)))
}

figure4 <- ggplot(data = sig_df) + 
   # geom_point(aes(x=log2( 1 + round((count_n1 + count_n2)/2 - abs(count_n1-count_n2)/2)), y=log2( 1 + round((count_n1+  count_n2)/2 + abs(count_n1-count_n2)/2 )), color=Gene, shape=condition), size = 3) + 
    geom_point(aes(x=log2(count_n1), y=log2(count_n2), color=Gene, shape=condition), size=3) + 
    ...
    geom_text_repel(aes(x=log2(count_n1), y= log2(count_n2), label = Gene, color= Gene)) + 
    scale_shape_manual(values=c(16,2)) + 
    coord_fixed(ratio=1, xlim=c(0,20), ylim=c(0,20)) + 
    geom_abline(slope = 1, intercept = 0) + 
    labs(x="log2(read counts) of copy 1" , y="log2(read counts) of copy 2", title ="paralog-specific read counts in two conditions") +
    guides(color=FALSE) + 
    theme_bw() +
    theme(
