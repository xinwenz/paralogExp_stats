...
exp35 <- read.table('35_expression.txt', header=T)
exp35$copy <- as.character(exp35$copy)

plotonegene <- function(x) {
ggplot(data=exp35) + 
    geom_bar(aes(x=strain, y = ntotal, fill=copy,alpha=condition), stat='identity', color="black",position = "dodge",data=exp35[exp35$geneName == x,]) +
    scale_fill_manual(values = c('1'='red','2'='darkcyan','3'='blue','5'='darkblue','7'='black','9'='orange'))+
